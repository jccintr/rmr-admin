...
const EditWorker = () => {
 ...
 const onUpdate = async (e) => {
  ...
  const response = await Api.updateWorker(loggedUser.token,fd,worker.id);
...
const handleFile = (e) => {
  ...
  return (
    ...
    <form  className='flex flex-col gap-4 mx-auto max-w-3xl' encType="multipart/form-data">
      ...
      <Button onClick={onUpdate} gradientMonochrome="info" disabled={isLoading}>{isLoading ? <Spinner size='sm'/>:'ATUALIZAR PROFISSIONAL'}</Button>
