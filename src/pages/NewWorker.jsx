...
const NewWorker = () => {
  ...
  //const {worker} = params.state;
  const [file, setFile] = useState(null);
  const [imagem, setImagem] = useState(null);
  const [imagemAtualizada, setImagemAtualizada] = useState(false);
  const [formData, setFormData] = useState({nome:'',email:'',password:'',password2:'',telefone:'',avatar: null,categoriaId:1,distritoId: 1,concelhoId: 1, isAdmin: false,nif:'',iban:''});
...
const onAdd = async (e) => {
  ...
  if (formData.email.trim().length===0) {
    setPublishError('Informe o email do profissional.');
    return;
  }
  ...
  if (formData.telefone.trim().length===0) {
    setPublishError('Informe o telefone do profissional.');
    return;
  }
  ...
  const response = await Api.addWorker(loggedUser.token,fd);
     
  if (response.status === 201){
     navigate('/?tab=profissionais');
  } else {
  ...
  return (
    ...
    <form  className='flex flex-col gap-4 mx-auto max-w-3xl' encType="multipart/form-data">
        ...
        <div className='flex flex-col gap-4 justify-between'>
            ...
            <Select   value={formData.categoriaId} id="categoria" onChange={(e) =>setFormData({ ...formData, categoriaId: e.target.value })}>
      ...
      <Button onClick={onAdd} gradientMonochrome="info" disabled={isLoading}>{isLoading ? <Spinner size='sm'/>:'ADICIONAR PROFISSIONAL'}</Button>
      <Button onClick={()=> navigate('/?tab=profissionais')} gradientMonochrome="failure" >CANCELAR</Button>
